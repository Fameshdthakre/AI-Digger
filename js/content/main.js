...
// Remove any stale listener from a previous extension context before re-registering.
// This handles extension reloads, SPA navigations, and repeat injections gracefully.
if (window._aiDiggerListener) {
    try { chrome.runtime.onMessage.removeListener(window._aiDiggerListener); } catch (e) { /* old context */ }
}

window._aiDiggerListener = function (message, sender, sendResponse) {
    if (message.action === 'PING') {
        sendResponse({ status: 'pong' });
    }
    else if (message.action === 'START_INSPECTOR_FOR_FIELD') {
...
chrome.runtime.onMessage.addListener(window._aiDiggerListener);